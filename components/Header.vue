<template>
  <div class="h-32 bg-white opacity-80 shadow-md bottom-0">
    ...
    <div class="mt-7 left-2 absolute right-1 top-0 text-right">
      ...
      <nuxt-link to="/memo" class="font-ui font-bold" v-if="loginFlag === true"
        >メモ帳</nuxt-link
      >
    </div>
    <!-- childFalseModalを子から受け取って親のfalseModalを発火させる -->
    <logout-modal class="left-0 fixed z-10 top-0 bottom-0 "  @childFalseModal="falseModal" v-if="modalFlag === true"></logout-modal>
  </div>
</template>
